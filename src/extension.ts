...
export function activate(context: vscode.ExtensionContext) {
	vscode.workspace.onDidSaveTextDocument(async (document: vscode.TextDocument) => {
		...
		const layoutXmlPattern = new RegExp(
			`^${workspaceFolder.uri.fsPath}${path.sep}res${path.sep}[^${path.sep}]+${path.sep}layout${path.sep}[^${path.sep}]+\.xml$`.replace(/\\/g, '\\\\'), "g"
		)
...
async function generateFiles(layoutXmlFile: string) {
	...
	const rootNode: ViewNode = result[Object.keys(result)[0]] as ViewNode
	const rootNodeClazz: string = Object.keys(result)[0] // 通常为 "CompositeView"
	...
	const flattenedLeafNodeMap = new Map<string, FlattenedViewNode>()
	const flattenedNonLeafNodeMap = new Map<string, FlattenedViewNode>()
...
function flatViewNode(viewNode: ViewNode, clazz: string, isRoot: boolean = true, parent?: FlattenedViewNode): Map<string, FlattenedViewNode> {
	const resultMap = new Map<string, FlattenedViewNode>()
	const { $, ...children } = viewNode
	const isLeaf = Object.keys(children).length === 0

	const flattenedNode: FlattenedViewNode = {
		id: $.id || "",
		clazz,
		isRoot,
		isLeaf,
	...
	for (const childClazz in children) {
		...
		if (Array.isArray(childNodes)) {
			childNodes.forEach(childNode => {
				const childResultMap = flatViewNode(childNode as ViewNode, childClazz, false, flattenedNode)
			...
			const childResultMap = flatViewNode(childNodes as ViewNode, childClazz, false, flattenedNode)
			childResultMap.forEach((value, key) => {
				resultMap.set(key, value)
			})
...
//  */
function modifyXmlNode(node: ViewNode, xmlFilePath?: string, isRoot = false) {
	// 判断节点是否为对象，避免操作无效节点
...
async function checkAndUpdateJsonFile(layoutXmlFile: string, leafNodeMap: Map<string, FlattenedViewNode>) {
	// 根据layoutXmlFile的路径找到对应的json文件
	const jsonFile = layoutXmlFile.replace(/\.xml$/, '.json')

	let jsonData: LayoutNode
	try {
	...
	leafNodeMap.forEach((value, key) => {
		if (!jsonData.hasOwnProperty(key)) {
			jsonData[key] = {
				type: 'RelativeLayout', // 使用默认的RelativeLayout
				params: {}
			}
			hasChanges = true
		}
...
async function checkAndUpdateThemeFiles(layoutXmlFile: string, flatViewNodeMap: Map<string, FlattenedViewNode>, rootNode: ViewNode) {
	...
	for (const themeFile of themeFiles) {
		...
		const themeNode = themeObject.theme as ThemeNode
...
function updateThemeObject(themeNode: ThemeNode, flattenedViewNodeMap: Map<string, FlattenedViewNode>, propertySetName: string) {
	...
	let newPropertySet: PropertySetNode = {
		$: { name: propertySetName }
	}
	if (Array.isArray(propertySets)) {
		const thisPropertySet = propertySets.find((propertySet: PropertyNode) => propertySet['$'].name === propertySetName)
		if (!thisPropertySet) {
	...
	const propertySetIds = newPropertySet.id as IdOrTagNode[]
	const propertySetTags = newPropertySet.tag as IdOrTagNode[]

	flattenedViewNodeMap.forEach((viewNode) => {
		const existsId = propertySetIds.find((propertySetId) => propertySetId?.$.name === viewNode.id)
		if (!existsId) {
			const idNode: IdOrTagNode = { _: " ", $: { name: viewNode.id } }
		...
		if (!existsTag && viewNode.tag) {
			const tagNode: IdOrTagNode = { _: " ", $: { name: viewNode.tag } }
...
async function generateTsClasses(layoutXmlFile: string, flattenedViewNodeMap: Map<string, FlattenedViewNode>) {
	...
	const workspaceFolder = vscode.workspace.getWorkspaceFolder(vscode.Uri.file(layoutXmlFile)) as vscode.WorkspaceFolder
	try {
 ...
 */
async function getImportMapping(nodeModulesPath: string): Promise<Map<string, string>> {
	const importMapping = new Map<string, string>()

	// 广度优先遍历node_modules文件夹, 记录包含package.json的文件夹
	const queue: string[] = [nodeModulesPath]
	const foldersWithPackageJson: string[] = []
 ...
 */
async function traverseFolder(basePath: string, currentPath: string, importMapping: Map<string, string>) {
...
interface BaseViewNode {
	[key: string]: ViewNode | ViewNode[]
}

type ViewNode = BaseViewNode & { $: ViewAttribute }
...
interface FlattenedViewNode {
	...
	propertySetName?: string
	layout?: string
	children: Map<string, FlattenedViewNode>
	parent?: FlattenedViewNode
}

// 主题相关类型声明
interface ThemeAttribute {
	name?: string
	extend?: string
}

interface PropertySetNode {
	$: { name: string }
	id?: IdOrTagNode | IdOrTagNode[]
	tag?: IdOrTagNode | IdOrTagNode[]
}

interface IdOrTagNode {
	_: string
	$: { name: string }
...
interface ThemeNode {
	$: ThemeAttribute
	"property-set"?: PropertySetNode | PropertySetNode[]
}
